...
For UI, mobile, or PWA changes, run `npm run test:browser` before reporting completion. It builds the shipped app and runs the production Playwright journey in Chromium, WebKit, and the two phone profiles. If local socket permissions block the command, rerun it with the required sandbox escalation; an `EPERM` startup error is not a test result.
